...
function sendBot(menu){
var request = require('request');
var send_uri = "";
var attachText = "";

if(command_args == "all")
  send_uri = "https://hook.dooray.com/services/[example_id_1]"; //[*커스텀수정필요]
else if(command_args == "team")
  send_uri ="https://hook.dooray.com/services/[example_id_2]";
else
  send_uri = "https://hook.dooray.com/services/[example_id_3]";
...
function googleOCR(fileName){
  visionClient.detectText(fileName)
  .then((results) => {
    const detections = results[0];

    sendBot(split(detections[0]));

    // console.log('Text:');
    // console.log(detections[0]);
    // detections.forEach((text) => console.log(text));
    // detections.forEach((text) => checkText(text));
  });
};

function imagecrop(){

  var offSetX = getDayOffset();
  var offSetY = 0;
  var crop_menu = 0;
